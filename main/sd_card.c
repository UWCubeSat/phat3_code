...
esp_err_t sd_card_mount(char* ret_dir_path, size_t ret_dir_path_size) {
    ...
    // Initialize the SPI bus connected to the SD card reader
    ...
    // Print SD info
    ESP_LOGD(LOG_TAG, "SD Card info: ");
    sdmmc_card_print_info(stdout, card);

    // Create a new unused folder
    for (int foldernum = 0; foldernum < 1000; foldernum++) {
    ...
    ESP_LOGE(LOG_TAG, "All 1000 possible SD directory names taken!");
    return ESP_FAIL;
}
