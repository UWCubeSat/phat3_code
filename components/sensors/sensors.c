...
esp_err_t sensors_init(void) {
    ...
    // ESP_RETURN_ON_ERROR(err, LOG_TAG, "Couldn't init SCD41 desc");
    // err = scd4x_wake_up(&scd41_dev);
    // ESP_RETURN_ON_ERROR(err, LOG_TAG, "Couldn't init SCD41 wake up");
    // err = scd4x_stop_periodic_measurement(&scd41_dev);
    // ESP_RETURN_ON_ERROR(err, LOG_TAG, "Couldn't init SCD41 stop measurement");
    // err = scd4x_reinit(&scd41_dev);
    // ESP_RETURN_ON_ERROR(err, LOG_TAG, "Couldn't init SCD41 reinit");
    // err = scd4x_start_periodic_measurement(&scd41_dev);
    // ESP_RETURN_ON_ERROR(err, LOG_TAG, "Couldn't init SCD41 start measurement");
...
// TODO: Split this function into different polling functions for each sensor
